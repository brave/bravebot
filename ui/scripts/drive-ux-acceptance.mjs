...
try {
  ...
  await page.getByRole('tab',{name:'History',exact:true}).click();const revision=page.locator('.memory-history [data-slot="accordion-item"]').first();await revision.getByRole('button').first().click();await snap('24-memory-history');await revision.getByRole('button',{name:'Review for restore'}).click();await page.getByRole('button',{name:'Save memory'}).click();await page.getByRole('tab',{name:'Read',exact:true}).click();await page.locator('.memory-readable').getByText('Saved preference',{exact:true}).waitFor();await snap('25-memory-restored');await page.keyboard.press('Escape')
