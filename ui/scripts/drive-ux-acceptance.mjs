...
try {
  ...
  const panel = (label) => page.locator(`.panel-pick[aria-label="${label}"]`)
  const ensurePanel = async (label, on=true) => {
  ...
  // Done is last in the dialog; Shift+Tab walks back to Refresh without fighting Base UI sentinels.
  await page.getByRole('button',{name:'Done',exact:true}).focus()
  await page.keyboard.press('Shift+Tab')
  assert.equal(await page.getByRole('button',{name:'Refresh',exact:true}).evaluate(e=>e===document.activeElement),true)
  await page.keyboard.press('Escape')
  await page.getByRole('dialog').waitFor({state:'hidden'})
  assert.equal(await page.getByRole('button',{name:'Permissions',exact:true}).evaluate(e=>e===document.activeElement),true)
  ...
  await ensurePanel('Changes', true);await ensurePanel('Files', false);await snap('29-waiting-write')
  ...
  await emit('tool.finished',{...activity,note:'Updated sample.txt',changes});await page.locator('.files .applied').waitFor();assert.equal(await panel('Changes').count(),1);await snap('31-applied-write');await done()
  ...
  await ensurePanel('Files', true);await page.getByRole('button',{name:'Search files',exact:true}).click();await page.getByRole('searchbox',{name:/Find files|Search/}).fill('sample');await page.getByRole('button',{name:'src/nested/sample.txt',exact:true}).waitFor();await snap('32-file-search')
  await ensurePanel('Files', false);await ensurePanel('Changes', true)
  ...
  await page.locator('.session').filter({hasText:'Plan the next iteration'}).click({timeout:15000});await page.locator('.session').filter({hasText:'Review the sample project'}).click();assert.equal(await page.locator('.entries').evaluate(e=>e.scrollTop),reading)
  ...
  await page.getByRole('tab',{name:'Bots',exact:true}).click();await page.locator('.bot').filter({hasText:'Review Bot'}).click();await snap('19-bot-overview')
  ...
  await page.getByRole('tab',{name:'Raw',exact:true}).click();await snap('22-memory-raw');await page.getByRole('button',{name:'Reset memory…',exact:true}).click();await snap('23-memory-reset');await page.getByRole('button',{name:'Reset saved memory'}).click()
  await page.getByRole('tab',{name:'History',exact:true}).click();await page.locator('.memory-history details').filter({hasText:'Saved preference'}).locator('summary').first().click();await snap('24-memory-history');await page.locator('.memory-history details').filter({hasText:'Saved preference'}).getByRole('button',{name:'Review for restore'}).first().click();await page.getByRole('button',{name:'Save memory'}).click();await page.getByRole('tab',{name:'Read',exact:true}).click();await page.locator('.memory-readable').getByText('Saved preference',{exact:true}).waitFor();await snap('25-memory-restored');await page.keyboard.press('Escape')
  await app.evaluate(({BrowserWindow})=>BrowserWindow.getAllWindows()[0].setSize(950,780));await page.setViewportSize({width:950,height:780});await page.getByRole('button',{name:'Context panel',exact:true}).click();await snap('26-narrow-drawer');await page.getByRole('button',{name:'Close context panel'}).click({force:true})
  ...
  await page.reload();await page.getByRole('tab',{name:'Sessions',exact:true}).click();await page.locator('.session').filter({hasText:'Plan the next iteration'}).click();await page.getByText('Backend setup needed',{exact:true}).waitFor();assert.equal(await composer.inputValue(),'Independent draft.');await snap('35-backend-not-ready');await page.getByRole('button',{name:'Diagnostics',exact:true}).click();await snap('38-diagnostics');await page.keyboard.press('Escape')
  ...
  await page.getByRole('tab',{name:'Bots',exact:true}).click();await page.locator('.bot').filter({hasText:'Review Bot'}).click();await page.locator('.bot-conversations button').filter({hasText:'Review the sample project'}).click();await page.locator('.transcript-head').count()
  ...
  await composer.fill('First turn failure fixture');await page.getByRole('button',{name:'Send',exact:true}).click();await emit('turn.error',{kind:'chat',category:'unauthorized',message:'401 credentials rejected',turn:1,id:'33333333-3333-4333-8333-333333333333'},'s-new');await page.getByText('Authentication failed',{exact:true}).waitFor()
  await page.getByRole('tab',{name:'Sessions',exact:true}).click();assert.equal(await page.locator('.session').filter({hasText:'First turn failure fixture'}).count(),1);await snap('40-first-turn-failure')
  await page.getByRole('tab',{name:'Bots',exact:true}).click();await page.locator('.bot').filter({hasText:'Review Bot'}).click();await page.getByRole('button',{name:'Edit bot and memory'}).click();await app.evaluate(()=>globalThis.ux.choose+='-copy');await page.getByRole('button',{name:'Duplicate into another project'}).click();await page.getByRole('dialog').waitFor({state:'hidden'});await page.locator('.bot').filter({hasText:'Review Bot copy'}).click();await snap('41-duplicated-bot');assert.match(await page.getByRole('dialog').innerText(),/-copy/);await page.keyboard.press('Escape')
