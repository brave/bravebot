...
function request(root: string, path: string, fields: Record<string, unknown>): Record<string, unknown> {
  const where = helperPaths('bravebot-ui-files', app.getAppPath())
  // Packaged, the binary ships beside the app and there is no workspace to look into.
  const binary = app.isPackaged ? where.packaged : where.development
