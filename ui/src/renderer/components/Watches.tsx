...
export function Watches({ session, onClose }: { session: string; onClose: () => void }): React.JSX.Element {
  ...
  return <Modal title="File watches" onClose={onClose} className="watch-settings">
    <div className="settings-heading"><h2>File watches</h2><Button variant="ghost" size="icon-sm" onClick={onClose} aria-label="Close file watches">×</Button></div>
    ...
    <ul className="watch-list">{listing?.watches.map(w => <li key={w.number}><div><strong>{w.path}</strong><p>{w.state === 'running' ? 'Automatic turn running' : listing.busy ? 'Waiting for this turn to finish' : 'Watching'} · Expires in {Math.max(1, Math.ceil(w.remainingSeconds / 3600))} hours</p><small>{w.armedBy ? `Armed by turn ${w.armedBy}` : 'Added by you'}</small></div><Button variant="outline" size="sm" disabled={busy} onClick={() => void request('watches.stop', { number: w.number })} aria-label={`Stop watching ${w.path}`}>Stop</Button></li>)}</ul>
    ...
    <div className="settings-actions"><Button variant="outline" disabled={busy || !listing?.watches.length} onClick={() => void request('watches.stop', { all: true })}>Stop all watches</Button><Button onClick={onClose}>Done</Button></div>
  </Modal>
}
