...
export function TurnNotices({ details, onDisclosure }: {
  ...
  return <Collapsible className="turn-notices" open={details.noticesOpen}
    ...
    <CollapsibleTrigger className="flex h-auto w-full items-center justify-start gap-1 px-0 py-1 text-xs"><span className={cn('chevron', details.noticesOpen && 'open')} aria-hidden="true">›</span>Turn notices · <Badge variant="secondary">{details.notices.length}</Badge></CollapsibleTrigger>
...
export function TurnFooter({ details, onDisclosure, onAudit }: {
  ...
  return <div className="turn-footer">
    {details?.status === 'complete' ? <Collapsible className="turn-statistics" open={details.statsOpen}
      ...
      <CollapsibleTrigger className="flex h-auto max-w-full items-center justify-start gap-1 px-0 py-1 text-left text-xs whitespace-normal"><span className={cn('chevron', details.statsOpen && 'open')} aria-hidden="true">›</span>{details.model ?? 'Model unavailable'} · {details.tokens === undefined ? 'Usage unavailable' : `${compact.format(details.tokens)} tokens`}</CollapsibleTrigger>
    ...
    <Button variant="link" className={cn('turn-audit-link h-auto min-h-0 px-0 py-1 text-left text-xs', details?.clean === false && 'has-refusal text-warn')}
