...
fn the_surfaces_the_bridge_needs_are_public() {
    ...
    let _: fn(&std::path::Path) -> Vec<bravebot_session::sessions::Summary> =
        bravebot_session::sessions::list;
    let _: fn(&std::path::Path, &str) -> Option<bravebot_session::sessions::Record> =
        bravebot_session::sessions::load;
    ...
    let _: fn(
        &bravebot_core::event::Event,
        Option<bravebot_agent::report::DelegateId>,
    ) -> serde_json::Value = bravebot_session::audit::as_json;
}
...
fn the_turn_engine_seams_are_public() {
    fn accepts<
        C: bravebot_agent::Confirmer,
        R: bravebot_agent::Reporter,
        S: bravebot_core::event::Sink,
    >() {
    }
    accepts::<
        bravebot_agent::Unattended,
        bravebot_agent::IgnoreReports,
        bravebot_core::event::NullSink,
    >();
}
