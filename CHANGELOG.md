## [0.9.0](https://github.com/brave/bravebot/releases/tag/v0.9.0)
 ...
 - Fixed a gateway that names no credential, such as a local Ollama, being refused for a missing one, with its model roster never asked for and `doctor` reporting a credential that has nothing to miss. Its requests go out with no `authorization` header. ([#355](https://github.com/brave/bravebot/issues/355))
