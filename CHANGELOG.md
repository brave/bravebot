## [0.9.0](https://github.com/brave/bravebot/releases/tag/v0.9.0)

 - Added a local Ollama as a model service, which needs no credential: a `provider` block naming its `baseURL` and no token is your word that none is needed, so requests go out with no `authorization` header, the model roster is asked for, and `doctor` says a credential is not needed rather than missing. ([#355](https://github.com/brave/bravebot/issues/355))
 - Added `bravebot --plain`, a session in lines: the same turns, conversation and questions with nothing drawn, so the terminal keeps raw mode, its own screen, the mouse and the scrollback. A line is a prompt, every question is answered `y` or not, and the reply goes to stdout on its own. It gives up the scroller, the slash commands, `@` and a session record to resume. ([#105](https://github.com/brave/bravebot/issues/105))
 - Added a check on quarantined content before you are asked to let any of it out: a second model instance reads the bytes and says whether they look like an attempt to give instructions, and you are shown that word and its reason beside the content. It authorises nothing, so the decision is still yours, and the check is a model request, so the content reaches the backend before you answer. ([#363](https://github.com/brave/bravebot/issues/363))
 - Added a `hooks.json` in `~/.bravebot`, which runs a command of your own when a turn starts, when one tool call finishes, or when the turn is over. The command is an argument vector, so no shell reads it, and a hook refuses nothing: its status is said to you and decides nothing. ([#92](https://github.com/brave/bravebot/issues/92))
 - Added a standing watch on a file, which outlives the turn that armed it and starts a turn of its own when the file changes, so a change is noticed with nothing running. Eight live at once for seven days each, `/watch` lists them, `/watch stop <n>` ends one, and a session holds one of a watch, a loop and a goal at a time. ([#264](https://github.com/brave/bravebot/issues/264))
 - Added `--json` and an exit status that says which failure a run hit: 2 a refused argument, 3 an unusable configuration, 4 an effect a gate refused, 5 a backend the request never reached. Each failure prints `BB1001` through `BB1005` in front of its message, so a script can classify a run whose words are in your language. ([#96](https://github.com/brave/bravebot/issues/96))
 - Added `SSL_CERT_FILE` and `SSL_CERT_DIR`, so a network that inspects TLS no longer fails every request with an unknown issuer while every other client on the machine works. They replace the shipped roots rather than adding to them, the proxy variables are read as the rest of the machine reads them, and `doctor` gains a `network` section naming the roots in force, the proxy, and the hosts it is not used for. ([#101](https://github.com/brave/bravebot/issues/101))
 - Added an administrator's file, in the directory the platform reserves for one, which pins where a request may go above anything in the environment: both endpoints, the AWS block and the gateway block. Its authority is the permissions on the path, a credential in it is ignored because everyone on the machine can read it, and `doctor` names the file and what it pinned. ([#94](https://github.com/brave/bravebot/issues/94))
 - Added `--settings <path>`, a fourth settings layer read after your home, project and local files, so one run can be configured differently without editing any of them. What it names wins per key and leaves the rest in force, and a path that names no file is refused. ([#93](https://github.com/brave/bravebot/issues/93))
 - Changed Linux confinement to need kernel 5.19 or later, which carries the right governing a move, so a confined program can rename a file inside a directory it was granted to write. That is how a compiler, a package manager and an editor save anything. An older kernel is refused rather than confined without the right. ([#4](https://github.com/brave/bravebot/issues/4))
 - Changed a first run with no model service configured to stop and say the three ways to set one up, each naming what to type or write, rather than running against an endpoint that will not serve it and looking like a poor agent. `doctor` says the same and fails. ([#333](https://github.com/brave/bravebot/issues/333))
 - Improved what a failed turn tells you: a fixed category rather than the backend's own diagnostics, a status of its own for a turn you stopped, and the reason stays on screen while you scroll or resize. ([#370](https://github.com/brave/bravebot/issues/370))
 - Changed what a rewind point keeps, so what a file in a directory nobody vouched for held is no longer written into the session record. It is recorded as contents the session did not keep, and a rewind after a resume names that path as one that did not go back. ([#51](https://github.com/brave/bravebot/issues/51))
 - Changed a processor's remark about a document to be drawn above the diff in the question about writing it, where it can be read against the bytes it claims to describe. It decides nothing: the approval is still given from the diff. ([#23](https://github.com/brave/bravebot/issues/23))
 - Changed the accent colour, which is what tells shell mode from ordinary mode, to be mixed against the background sensed at startup rather than taking one of the sixteen slots a terminal scheme repaints. ([#180](https://github.com/brave/bravebot/issues/180))
 - Changed the hint line to say the context is not yet measured, rather than drawing nothing there, which looked the same as a terminal too narrow for the figure and the same as a reading that had stopped working. ([#113](https://github.com/brave/bravebot/issues/113))
 - Fixed a gateway that refuses an effort level refusing every request the turn could make, rather than the level being given up and the turn carrying on without it. ([#361](https://github.com/brave/bravebot/issues/361))
 - Fixed two things on Windows: the environment block left the OS version out, and Ctrl-G reported no editor on a machine where one is installed, since a bare name like `notepad` or `code` matched nothing on the PATH. The four terminal editors are tried there now too, so Git for Windows' vim is found. ([#160](https://github.com/brave/bravebot/issues/160))
 - Fixed Ctrl-V doing nothing while a turn runs, so the clipboard is read and a picture pasted with the terminal's own chord is attached rather than discarded. ([#170](https://github.com/brave/bravebot/issues/170))
 - Fixed `/status` replacing all but six vouched commands with a count, which left a standing permission held with nothing anywhere saying so. ([#57](https://github.com/brave/bravebot/issues/57))
 - Fixed a delegate's block drawing its last lines rather than its last calls, which hid calls and undercounted the rest, and the delegate view being pulled off the row you opened it on every time the turn reported anything. ([#182](https://github.com/brave/bravebot/issues/182))
 - Fixed answering a `/btw` writing a note into the transcript and streaming the answer where the reply is drawn, so an aside neither half of which the agent saw no longer reads as part of the conversation. ([#183](https://github.com/brave/bravebot/issues/183))
 - Fixed the scroller's search counting the lines that hold a match rather than the matches, which said `1 of 1` for two on one line and left the second unreachable by `n` and `N`. ([#179](https://github.com/brave/bravebot/issues/179))
 - Fixed the model picker's cursor moving to another model as you typed, and a service's heading being drawn twice at some scroll offsets. ([#181](https://github.com/brave/bravebot/issues/181))
 - Fixed a message your translation leaves out being pluralised by the rules of the language you asked for rather than the language it is written in, so a French screen no longer reads `sent back 0 time`. ([#159](https://github.com/brave/bravebot/issues/159))
 - Fixed a session record whose per-turn breakdown did not add up to its own total, where a `/btw` or a `/manifest` ran before the first turn and its tokens were charged to no turn at all. ([#188](https://github.com/brave/bravebot/issues/188))
...
## [0.8.0](https://github.com/brave/bravebot/releases/tag/v0.8.0)
 ...
 - Fixed an imported subscription being read as absent where an interrupted import had left the file empty, which stopped the subscription being spent with nothing said and no symptom but the agent getting worse. ([#192](https://github.com/brave/bravebot/issues/192))
