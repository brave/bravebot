...
#
# Not in check-all yet, because it fails on this tree: `Labelled::new` has no `guards` entry in
# docs/specs/labels.md, where `Labelled::trusted` has one. Adding this target before the second one
# lands would make a red check-all the normal state, which is how a check stops being read. Add it
# to check-all in the change that pins `Labelled::new`.
