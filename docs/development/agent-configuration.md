...
Cursor and bravebot both read subagents, so `agents/agents/` links into `.cursor/agents` and
`.bravebot/agents` as well. bravebot calls one a delegate definition and ignores the frontmatter
keys it has no use for ([DELEGATE-19](../specs/delegation.md#DELEGATE-19)), which is what lets one
directory serve both rather than each tool needing a copy. Slash commands Cursor dropped in favour
of skills, so `agents/commands/` links into `.claude/` alone. Neither directory has to exist:
whatever is present is linked, and a directory added later needs no change to the script.
...
`make init` does not grant trust. bravebot loads a workspace skill or delegate definition only
from a path a person vouched for ([TRUST-1](../specs/trust-map.md)), and a script granting that on
your behalf is the inference that clause forbids, so expect to be asked about `.bravebot/skills`
and `.bravebot/agents` the first time you start it in this tree.
