...
| `bravebot-agent` | Task execution: the tools, the turn loop, and the standing watches a session keeps | `core`, `aichat`, `bedrock`, `config`, `i18n`, `lsp`, `mcp`, `net`, `sandbox`, `skus` | Carries labelled values and must not inspect them. `exec` stays argv-only, and `shell` runs only a line a person typed. Offers a started MCP server's tools only once a person vouched for its list as drawn, and calls one only on their answer, a recorded one, a rule, or the mode they named that answers for them. See [mcp-servers.md](mcp-servers.md) |
...
| `bravebot-tui` | The interactive terminal interface | `core`, `agent`, `aichat`, `config`, `i18n`, `net`, `sandbox`, `session`, `stamp`, `ui-bridge` | Presentation. Reaches `ui-bridge` only in tests, to exercise terminal/bridge session handoffs. May display released content, always inside a margin it draws itself. Owns the clipboard and shell mode, both of which are gestures a person made. Owns the terminal itself, so it may read the tty directly to ask the terminal about itself; what comes back describes the terminal and never enters a turn |
| `bravebot-cli` | Command-line entry point | `core`, `agent`, `config`, `i18n`, `mcp`, `net`, `sandbox`, `session`, `skus`, `stamp`, `tui` | Presentation. Where nobody can be asked, effects are refused rather than applied unseen. Writes the one file that declares an MCP server, only from a command a person typed, and records an approval of one only on their answer at a terminal. Starts the servers a session was asked for, each only on an answer a person gave or recorded or under the mode they named that answers for them, a local one only confined and a remote one only through the egress gate. See [mcp-servers.md](mcp-servers.md) |
| `bravebot-mcp` | Model Context Protocol client: the extension boundary for tools | `core`, `config`, `net`, `sandbox` | An opaque call erases the routing/content split, so primitives stay native rather than moving behind it. Reaches `config` for the one rule of what a tool's word may be, so the name a list offers and the name a standing answer records are read alike |
...
| `bravebot-sandbox` | OS-level confinement: what a subprocess may reach, what a crash of this process may leave behind, and which of its pages swap may take | none | Confines processes running code we did not write. A processor's caller is our own code, so it is not what this confines. Also holds what a process says to the kernel about its own memory image, which is a platform call rather than a policy and belongs with the other ones: the crate that owns a credential forbids `unsafe`, so it can clear a buffer and asks this one for anything the kernel has to be told |
| `bravebot-config` | Environment-derived configuration for the backend | `sandbox` | The configuration surface, on the same footing as the endpoint and the model. Reaches `sandbox` for the one platform call a credential needs, holding it in pages the kernel keeps out of swap, since this crate forbids `unsafe`. Reads the settings layers, including one a checkout may carry, and hands on rule text without matching a rule. Reads an MCP server's declaration from the person's own directory and from no layer, and records a layer that tries to make one rather than acting on it. Reads and writes the standing answers about a server's tools in the same directory. See [backends.md](backends.md) and [mcp-servers.md](mcp-servers.md) |
...
and which also makes the calls on this process's own limits and pages that a crate holding a
credential forbids itself ([CRED-23](credential-protection.md#CRED-23)),
