...
| `bravebot-ui-files` | Reads and writes the files the graphical front end is asked to open, under a directory it is handed | none | Reads and writes only under a directory its caller pins, resolving each path component relative to it and never following a link, so a path cannot leave the tree a person opened. Runs no command and hands nothing to a turn. Which directories may be pinned is [trust-map.md](trust-map.md)'s question rather than this document's |
