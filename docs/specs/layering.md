...
| `bravebot-ui-files` | Reads a file the graphical front end was asked to show | none | Reads and writes only under a directory its caller pins, resolving each path component relative to it and never following a link, so a path cannot leave the tree a person opened. Runs no command and hands nothing to a turn. Which directories may be pinned is [trust-map.md](trust-map.md)'s question rather than this document's |
