...
- **The third is reading a verdict out of a check.** A second model is shown quarantined content (a
  ...
  - **Force the word `safe`.** With auto-vetting off, which is the default and every session
    ...
    its own.
  - **Put the key that turns auto-vetting on in front of the person.** The vetting prompt's
    standing key is drawn only where the check found nothing, so forcing `safe` is what offers it
    ...
    It is drawn only on that prompt, and the other direction is held shut, since a warning or a
    check that could not be made offers nothing. The reason the offer is put where the mode would
    have saved a keystroke rather than left out of the interface is that the alternative is
    somebody editing a settings file to get it, which is a decision made further from the thing it
    is about.
  - **Force the word `safe` where somebody turned auto-vetting on.** Then it promotes, and nobody
    ...
    nothing becomes routing, with no trust rule written, and only on the route
    [CHECK-12](vetting.md#CHECK-12) covers, so releasing what a program printed and vouching for a
    path are still asked about. The slot is still the planner's choice and not the content's.
