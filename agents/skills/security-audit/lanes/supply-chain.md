...
advisories, licences, sources and duplicate versions, and every workflow step in the tree names a
commit. Do not report what those already fail on, and do not report a rule as a review habit when it
could be a check: where you find one, the finding is a request for the check.
