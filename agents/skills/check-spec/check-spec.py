...
UNDOCUMENTED_FILE = Path("undocumented-clauses.txt")

UNDOCUMENTED_HEADER = """\
# Spec clauses whose documented-by value is none or not yet mapped.
# Generated by make write-undocumented. Document a clause rather than editing here: while the two
# disagree, make check-spec fails.
# Source: docs/specs/**/*.md, one line per clause, as path:CLAUSE-ID: heading.
# Withdrawn clauses carry no documented-by line at all and are not listed.
# No count is kept here. Two branches that each document a clause both edit the count, so it conflicts
...
def check_documentation(spec, website_docs=Path("docs/website/docs")):
    for clause in spec.clauses:
        if clause.withdrawn:
            continue
        if not clause.documented_by:
            yield finding(
                spec.rel,
                WARNING,
                "clause-undocumented",
                f"`{clause.id}` names no documentation page: every clause carries a `documented-by` line",
                clause=clause.id,
                evidence=f"{spec.rel}:{clause.line}",
                fix="document the behaviour under docs/website/docs/, or say none with the reason in brackets",
            )
            continue
        for reference in clause.documented_by:
            if reference.startswith("none"):
                ...
                if not (bracketed and len(rest) > 2):
                    yield finding(
                        ...
                        ERROR,
                        "none-unexplained",
                        f"`{clause.id}` says none without saying in brackets why it needs no user-facing documentation",
                        clause=clause.id,
                        evidence=f"{spec.rel}:{clause.line}",
                    )
                else:
                    yield finding(
                        ...
                        WARNING,
                        "clause-undocumented",
                        f"`{clause.id}` is `documented-by: none`, so no website page explains it",
                        clause=clause.id,
                        evidence=f"{spec.rel}:{clause.line}",
                    )
                continue
            page_ref = reference.split("#", 1)[0].strip()
            target = website_docs / page_ref
            if not target.is_file():
                yield finding(
                    spec.rel,
                    ERROR,
                    "documented-by-missing",
                    f"`{clause.id}` names `{reference}`, but `{target}` does not exist",
                    clause=clause.id,
                    evidence=f"{spec.rel}:{clause.line}",
                    fix="create the page or correct the path under docs/website/docs/",
                )


def undocumented_clauses(specs, website_docs=Path("docs/website/docs")):
    undocumented = {
        (found["spec"], found["clause"])
        for spec in specs
        for found in check_documentation(spec, website_docs)
        if found["kind"] == "clause-undocumented"
    }
    return [
        f"{spec.rel}:{clause.id}: {clause.title}"
        for spec in specs
        for clause in spec.clauses
        if (spec.rel, clause.id) in undocumented
    ]


def render_undocumented(lines):
    return UNDOCUMENTED_HEADER + "\n" + "".join(f"{line}\n" for line in lines)


def check_undocumented_file(specs, website_docs=Path("docs/website/docs")):
    generated = render_undocumented(undocumented_clauses(specs, website_docs))
    committed = UNDOCUMENTED_FILE.read_text(encoding="utf-8") if UNDOCUMENTED_FILE.exists() else ""
    ...
    detail = []
    if live - listed:
        detail.append(f"no page documents {', '.join(sorted(live - listed))}")
    if listed - live:
        detail.append(f"a page now documents {', '.join(sorted(listed - live))}")
    yield finding(
        str(UNDOCUMENTED_FILE),
        ERROR,
        "undocumented-list-stale",
        f"{UNDOCUMENTED_FILE} is not the list this tree generates",
        evidence="; ".join(detail) or "the same clauses, written another way",
        fix="make write-undocumented, and commit what it writes",
    )
...
def main():
    ...
    parser.add_argument(
        "--write-undocumented", action="store_true", help=f"write {UNDOCUMENTED_FILE} from the specs"
    )
    ...
    if (args.write_unverified or args.write_undocumented) and partial_selection(args.specs, args.changed):
        print("unverified and undocumented lists require every spec: drop the filter", file=sys.stderr)
        return 2
    ...
    if args.write_unverified:
        ...
        return 0
    if args.write_undocumented:
        lines = undocumented_clauses(specs)
        UNDOCUMENTED_FILE.write_text(render_undocumented(lines), encoding="utf-8")
        print(f"{UNDOCUMENTED_FILE}: {len(lines)} clauses")
        return 0
    ...
    if len(chosen) == len(specs):
        ...
        findings.extend(check_undocumented_file(specs))
